...
// Helpers for the new Bikes tab (Tab 3)
const classifyReport = (rec: any) => {
  ...
  if (status === 'Vandalizada' || obs.includes('vandalizada') || obs.includes('vandalismo') || obs.includes('danific') || obs.includes('quebrad') || obs.includes('quadro pintado') || obs.includes('pneu solto') || obs.includes('sem pezinho') || obs.includes('sem cesto') || obs.includes('sem locker') || obs.includes('sem identificação')) {
    return 'Vandalizada';
  }
  if (obs.includes('locker') || obs.includes('box') || obs.includes('lock')) {
    return 'Manutenção Locker';
  }
  if (obs.includes('bateria') || obs.includes('baixa') || (rec.bateria !== undefined && rec.bateria !== '' && Number(rec.bateria) < 50)) {
    return 'Bateria Baixa';
  }
  if (status === 'Filial' || obs.includes('manutenção') || obs.includes('manutencao') || obs.includes('reparo') || obs.includes('oficina') || obs.includes('mecanic') || obs.includes('técnic')) {
    return 'Manutenção Bicicleta';
  }
  if (status === 'Filial') {
    return 'Manutenção Bicicleta';
  }
...
export const AnalyticalDashboard: React.FC<AnalyticalDashboardProps> = ({ onClose, apiCall }) => {
  const [activeTab, setActiveTab] = useState<'drivers' | 'maintenance' | 'bikes'>('drivers');
  ...
  // Fetch Finalizacao reports for Tab 3 (Bikes)
  ...
  const processedBikesData = useMemo(() => {
    ...
    const bikesFilialCounts: Record<string, { count: number; lastObs: string[] }> = {};

    finalizacaoRecords.forEach(rec => {
      const ts = rec.timestamp;
      if (!ts) return;
      
      const periods = getRecordPeriods(ts);
      if (!periods) return;
      ...
      if (matchesTimeframe) {
        const status = rec.status || '';
        const obs = rec.observacao || rec.observation || '';
        const pat = String(rec.patrimonio || rec.bikeNumber || '').trim();

        // Estações que mais recebem bikes remanejadas
        if (status === 'Estação' && obs && obs !== 'Fora da Estação' && obs !== 'Buscando...') {
          stationsCounts[obs] = (stationsCounts[obs] || 0) + 1;
        }

        // Top 10 bikes que retornam para Recolhida Filial
        if (status === 'Filial' && pat) {
          if (!bikesFilialCounts[pat]) {
            bikesFilialCounts[pat] = { count: 0, lastObs: [] };
          }
          bikesFilialCounts[pat].count++;
          if (obs && !bikesFilialCounts[pat].lastObs.includes(obs)) {
            bikesFilialCounts[pat].lastObs.push(obs);
          }
        }
      }
    });

    const topStations = Object.entries(stationsCounts)
  ...
  }, [finalizacaoRecords, bikeTimeRange]);
  ...
  const exportBikesSummaryCSV = () => {
    ...
    link.setAttribute('download', `resumo_coleta_bicicletas.csv`);
  ...
  useEffect(() => {
    ...
    fetchBikesData();
  }, [fetchData, fetchMaintenanceData, fetchBikesData]);
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 z-50 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 lg:p-8">
        ...
        {/* TAB 3: BICYCLES ANALYTICAL DASHBOARD */}
        {activeTab === 'bikes' && (
          <div className="space-y-6 animate-fade-in">
            
            {/* Controls and Header Info */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-gray-900">Análise de Bicicletas Recolhidas</h2>
                  <p className="text-xs text-gray-500">Mapeamento de motivos, recolhas filial, readequações e bikes críticas.</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={exportBikesSummaryCSV}
                  className="px-3 py-2 bg-white hover:bg-gray-50 text-gray-700 border rounded-lg text-xs font-bold shadow-sm transition-all flex items-center gap-1.5"
                  id="btn_export_bikes_summary"
                >
                  <Download className="w-4 h-4" /> Exportar CSV
                </button>

                <button
                  onClick={fetchBikesData}
                  className="p-2 bg-white hover:bg-gray-50 text-gray-700 border rounded-lg shadow-sm transition-all"
                  title="Recarregar"
                  id="btn_reload_bikes"
                >
                  <RefreshCw className={`w-4 h-4 ${bikesLoading ? 'animate-spin' : ''}`} />
                </button>
              </div>
            </div>
            ...
            {bikesLoading ? (
              <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-dashed">
                ...
                <p className="text-sm text-gray-500 mt-4 font-medium">Buscando histórico de finalizações no Firestore...</p>
              </div>
            ) : bikesError ? (
              <div className="p-6 bg-red-50 border border-red-200 rounded-xl text-center text-red-700">
                ...
                <p className="font-bold text-sm">{bikesError}</p>
                <button onClick={fetchBikesData} className="mt-3 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg transition-all">
                  Tentar Novamente
                </button>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                  {Object.entries(processedBikesData.reasonsCounts).map(([name, counts]) => {
                    ...
                    let headerColor = 'text-blue-950';

                    if (name === 'Manutenção Locker') {
                      icon = <HelpCircle className="w-5 h-5 text-indigo-600" />;
                      colorTheme = 'border-indigo-100 bg-indigo-50/10';
                      bgIcon = 'bg-indigo-50 text-indigo-600';
                      headerColor = 'text-indigo-950';
                    } else if (name === 'Bateria Baixa') {
                      icon = <Battery className="w-5 h-5 text-amber-600" />;
                      colorTheme = 'border-amber-100 bg-amber-50/10';
                      bgIcon = 'bg-amber-50 text-amber-600';
                      headerColor = 'text-amber-950';
                    } else if (name === 'Vandalizada') {
                      icon = <AlertTriangle className="w-5 h-5 text-red-600" />;
                      colorTheme = 'border-red-100 bg-red-50/10';
                      bgIcon = 'bg-red-50 text-red-600';
                      headerColor = 'text-red-950';
                    } else if (name === 'Não Encontrada') {
                    ...
                    return (
                      <div key={name} className={`bg-white p-5 rounded-xl border shadow-sm flex flex-col justify-between transition-all hover:shadow-md ${colorTheme}`}>
                        <div className="flex items-start justify-between">
                          <span className={`text-xs font-extrabold uppercase tracking-widest ${headerColor}`}>{name}</span>
                          <span className={`p-1.5 rounded-lg ${bgIcon}`}>{icon}</span>
                        </div>
                        <div className="mt-4">
                  ...
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={bikeChartData}>
                        ...
                        <Bar dataKey="Quantidade" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={40}>
                          {bikeChartData.map((entry, index) => {
                            let barColor = '#3b82f6'; // Manutenção Bicicleta
                            if (entry.name === 'Manutenção Locker') barColor = '#6366f1';
                ...
                {/* Top Estacoes Remanejadas + Top 10 Bikes Criticadas */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white rounded-xl shadow-sm border overflow-hidden flex flex-col justify-between">
                    <div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs sm:text-sm">
                          <thead className="bg-gray-50 text-gray-500 text-[10px] sm:text-xs uppercase tracking-wider border-b">
                            <tr>
                              <th className="px-5 py-3 font-bold">Estação</th>
                              <th className="px-5 py-3 font-bold text-right">Qtd. Bikes Remanejadas</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {processedBikesData.topStations.length > 0 ? (
                              processedBikesData.topStations.map((row, idx) => (
                                <tr key={idx} className="hover:bg-gray-50/80 transition-all">
                                  <td className="px-5 py-3.5 font-bold text-gray-900 flex items-center gap-2">
                                    <span className="w-5 h-5 rounded-full bg-slate-100 text-slate-700 text-[10px] font-black flex items-center justify-center">{idx + 1}</span>
                                    {row.name}
                                  </td>
                                  <td className="px-5 py-3.5 font-black text-right text-emerald-600">{row.count}</td>
                                </tr>
                              ))
                            ) : (
                              <tr>
                                <td colSpan={2} className="px-5 py-10 text-center text-gray-400 font-medium italic">
                                  Nenhum remanejamento registrado para estação neste período.
                                </td>
                              </tr>
                            )}
                          </tbody>
                    ...
                    <div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs sm:text-sm">
                          <thead className="bg-gray-50 text-gray-500 text-[10px] sm:text-xs uppercase tracking-wider border-b">
                            <tr>
                              <th className="px-5 py-3 font-bold">Nº Patrimônio</th>
                              <th className="px-5 py-3 font-bold">Últimas Observações</th>
                              <th className="px-5 py-3 font-bold text-right">Retornos Filial</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {processedBikesData.topBikes.length > 0 ? (
                              processedBikesData.topBikes.map((row, idx) => (
                                <tr key={idx} className="hover:bg-gray-50/80 transition-all">
                                  <td className="px-5 py-3.5 font-black text-gray-900 flex items-center gap-2">
                                    <span className="w-5 h-5 rounded-full bg-red-50 text-red-700 text-[10px] font-black flex items-center justify-center">{idx + 1}</span>
                                    Bike {row.bike}
                                  </td>
                                  <td className="px-5 py-3.5 text-gray-500 max-w-[200px] truncate" title={row.lastObs}>
                                    {row.lastObs}
                                  </td>
                                  <td className="px-5 py-3.5 font-black text-right text-red-600">{row.count}</td>
                                </tr>
                              ))
                            ) : (
                              <tr>
                                <td colSpan={3} className="px-5 py-10 text-center text-gray-400 font-medium italic">
                                  Nenhum retorno para filial registrado neste período.
                                </td>
                              </tr>
                            )}
                          </tbody>
